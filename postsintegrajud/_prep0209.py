...
os.makedirs('work0209', exist_ok=True)
zipfile.ZipFile('gerador-integrajud.zip').extractall('work0209')
print('unzip ok')

for f in ['machine.py', 'reel.py', 'story_ad.py', 'fundo_numerico.py',
          'retrato.py', 'repor_fotos.py', 'ffmpeg.exe']:
    shutil.copy(f, os.path.join('work0209', f))
print('scripts vivos da raiz copiados por cima')

src, dst = 'fotos_banco', os.path.join('work0209', 'fotos_banco')
